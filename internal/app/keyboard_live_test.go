...
func TestLearnLiveKeys(t *testing.T) {
	...
	learnLiveKeys(learned, []ebiten.Key{ebiten.KeyA}, []rune{'a'}, none)
	...
	learnLiveKeys(learned, []ebiten.Key{ebiten.KeySpace}, []rune{' '}, none)
	...
	before := len(learned)
	learnLiveKeys(learned, nil, []rune{'x'}, none)
...
func TestLearnLiveKeys_ExcludesHeldRepeats(t *testing.T) {
	...
	learnLiveKeys(learned, []ebiten.Key{ebiten.KeyB}, []rune{'a', 'b'}, pressed)
...
func TestLearnLiveKeys_PurgesStaleOnNoChar(t *testing.T) {
	...
	learnLiveKeys(learned, []ebiten.Key{ebiten.KeyE}, nil, func(ebiten.Key) bool { return false })
...
func TestResolveKeys_HeldCharKey(t *testing.T) {
	...
	in := resolveKeys(pressedSet(ebiten.KeyA), learned, false, nil)
	if !in.Keys[mo5] {
		t.Errorf("touche MO5 %d devrait être tenue", mo5)
	}

	// Relâchée → non tenue.
	in = resolveKeys(pressedSet(), learned, false, nil)
	if in.Keys[mo5] {
...
func TestResolveKeys_AntiDoubleShift(t *testing.T) {
	...
	mo5, _, _ := keyboard.CharToMO5Key('a')
	learned := map[ebiten.Key]liveKey{ebiten.KeyA: {mo5: mo5, shift: false}}
	in := resolveKeys(pressedSet(ebiten.KeyA, ebiten.KeyShiftLeft), learned, false, nil)
	...
	in = resolveKeys(pressedSet(ebiten.KeyB), learned2, false, nil)
...
func TestResolveKeys_SuppressesAltGrWhenCharHeld(t *testing.T) {
	...
	in := resolveKeys(pressedSet(ebiten.KeyDigit0, ebiten.KeyAltLeft, ebiten.KeyControlLeft), learned, false, nil)
...
func TestResolveKeys_PhysicalShiftPositionalWhenNoChar(t *testing.T) {
	...
	in := resolveKeys(pressedSet(ebiten.KeyShiftLeft), map[ebiten.Key]liveKey{}, false, nil)
	if !in.Keys[keyboard.Mo5KeyShift] {
...
func TestResolveKeys_SpecialKeysHeld(t *testing.T) {
	...
	in := resolveKeys(pressedSet(ebiten.KeyArrowRight), map[ebiten.Key]liveKey{}, false, nil)
...
func TestResolveKeys_InjectingIgnoresLiveAndFiltersModifiers(t *testing.T) {
	...
	// tickKeys appliqués.
	in := resolveKeys(pressedSet(ebiten.KeyA, ebiten.KeyShiftLeft, ebiten.KeyControlLeft), learned, true, []int{0x12})
	if in.Keys[mo5] {
