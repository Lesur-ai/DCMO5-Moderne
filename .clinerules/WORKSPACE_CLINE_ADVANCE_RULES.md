...
Graph Memory is connected for `{SPACE}` (which lives on the Live Memory server `{LIVE_MCP_SERVER}`):
...
1. Use `graph_status(space_id="{SPACE}")` to verify connectivity before any Graph operation.
...
8. If a push or query times out, check `graph_status` before retrying; partial ingestion may already have occurred.
...
Parler en français de manière concise et didactique. Appeler l'utilisateur par son prénom.
Ne jamais mettre de valeurs en dur dans le code (timeouts, limites → config/env).
Pour les longues commandes terminal, écrire un petit script plutôt que risquer la troncature de l'éditeur.
